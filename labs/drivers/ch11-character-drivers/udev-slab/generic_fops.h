...
#ifndef CHAPTER_11_UDEV_SLAB_DRIVER_
#define CHAPTER_11_UDEV_SLAB_DRIVER_
...
static int generic_cdev_open(struct inode *inode, struct file *file)
{
	...
	dev_info(udev_device, "generic_cdev_open(): Opening device %s: MAJOR %d MINOR %d\n",
			UDEV_SLAB_NAME, MAJOR(dev_num), MINOR(dev_num));
	dev_info(udev_device, "module_refcount=%d\n", (int)module_refcount(THIS_MODULE));
...
static int generic_cdev_release(struct inode *inode, struct file *file)
{
	...
	dev_info(udev_device, "generic_cdev_release(): Releasing device: %s\n", UDEV_SLAB_NAME);
	dev_info(udev_device, "generic_cdev_release(): MSG: %s\n", obj->buffer);
		...
		struct file *file, char __user *buf, size_t lbuf, loff_t *ppos)
{
	struct obj_buf *obj = (struct obj_buf *)file->private_data;
	int ret;
	dev_info(udev_device, "generic_cdev_read(): Starts lbuf=%d, *ppos=%d, nbytes=%d\n",
		(int)lbuf, (int)*ppos, (int)obj->nbytes);

	ret = simple_read_from_buffer(buf, lbuf, ppos, obj->buffer, obj->nbytes);

	dev_info(udev_device, "generic_cdev_read(): ret %d MSG: %s\n", ret, obj->buffer);
...
{
	struct obj_buf *obj = (struct obj_buf *)file->private_data;
	dev_info(udev_device, "generic_cdev_write(): Starts lbuf=%d, *ppos=%d\n",
		(int)lbuf, (int)*ppos);
...
loff_t generic_cdev_llseek(struct file *file, loff_t offset, int whence)
{
	...
	dev_info(udev_device, "generic_cdev_llseek(): offset=%d\n", (int)retval);

out:
	return retval;
}

#endif // CHAPTER_11_UDEV_SLAB_DRIVER_
